...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=visca \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=viscad \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/viscad ./cmd/viscad
endif

build-linux:
	mkdir -p $(BUILDDIR)
	docker build --no-cache --tag onchainengineer/visca ./
	docker create --name temp onchainengineer/visca:latest
	docker cp temp:/usr/local/bin/viscad $(BUILDDIR)/
	docker rm temp

build-linux-with-shared-library:
	mkdir -p $(BUILDDIR)
	docker build --tag onchainengineer/visca-shared ./ -f ./shared.Dockerfile
	docker create --name temp onchainengineer/visca-shared:latest
	docker cp temp:/usr/local/bin/viscad $(BUILDDIR)/
	docker cp temp:/lib/libwasmvm.so $(BUILDDIR)/
	docker rm temp

install: go.sum 
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/viscad
...
draw-deps:
	...
	@goviz -i ./cmd/viscad -d 2 | dot -Tpng -o dependency-graph.png

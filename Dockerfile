...
# See https://github.com/onchainhub/wasmvm/releases
...
RUN addgroup visca \
    && adduser -G visca -D -h /visca visca

WORKDIR /visca

COPY --from=go-builder /code/build/viscad /usr/local/bin/viscad
...
CMD ["/usr/local/bin/viscad", "version"]
